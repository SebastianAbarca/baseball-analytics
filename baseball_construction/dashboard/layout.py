...
def identity_tab() -> dbc.Tab:
    ...
    return dbc.Tab(label="Identity", tab_id="tab-identity", children=[
        ...
        # Moved here from Compare. It is a LEAGUE-level view — every team
        # ranked, each with its nearest neighbours by fingerprint similarity
        # — so it sat oddly in a tab that now compares individual players.
        # Identity is where a team is placed against the rest of the league.
        dbc.Card([
            dbc.CardHeader(
    ...
    return dbc.Tab(label="Scout", tab_id="tab-scout", children=[
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                ...
                dbc.Row([
                    dbc.Col([
                        html.Label("Restrict comps to", **_lbl),
                        dcc.Dropdown(id="comp-require", multi=True,
                                     placeholder="any handedness or slot",
                                     className="dash-dropdown"),
                    ], md=9),
                ], className="g-2 mt-1"),
                html.Small(
